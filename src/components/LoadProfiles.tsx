...
export default function LoadProfiles({ 
  ...
  const [activeAnalysisTab, setActiveAnalysisTab] = useState<'overview' | 'spot' | 'fixed'>('overview');
  ...
  // Shared building blocks (used both in comparison mode and inside the
  // single-profile "Übersicht" tab). Extracted so the markup and all
  // calculations stay identical regardless of layout.
  const kpiCards = (
    <ProfileKPICards
  ...
  return (
    <div className="p-6 space-y-8">
        ...
        <div className="flex space-x-3">
          <PDFExportButton
            ...
            onBeforeExport={async () => {
              // Ensure the load profile chart (captured by ID for the PDF) is
              // visible before html2canvas runs, even if another tab is active.
              if (!isComparisonMode) {
      ...
      {isComparisonMode ? (
        /* Comparison mode: unchanged stacked layout */
        <>
          {kpiCards}
        ...
        /* Single profile mode: tabbed layout.
           All panels stay mounted (inactive ones hidden via CSS) so every
           calculation, callback and PDF export data stays identical. */
        <>
          {/* Tab navigation */}
          <div className="flex gap-2 border-b border-gray-200">
            {([
              { id: 'overview', label: 'Übersicht' },
              { id: 'spot', label: 'Spot-Preis Analyse' },
              { id: 'fixed', label: 'Fixpreis Analyse' },
            ] as const).map((tab) => (
              <button
                key={tab.id}
          ...
          {/* Spot Price Analysis */}
          <div className={activeAnalysisTab === 'spot' ? '' : 'hidden'}>
            <SpotPriceAnalysis
              profile={selectedProfiles[0]}
          ...
          <div className={activeAnalysisTab === 'fixed' ? '' : 'hidden'}>
            <FixedPriceAnalysis
              profile={selectedProfiles[0]}
